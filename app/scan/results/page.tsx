...
export default function ScanResults() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(planTitle, pageWidth / 2, 25, { align: 'center' });
    ...
    yPos += 3;
    addText(`Type: ${treatmentData.furnitureType}`, margin + 5, 11);
    addText(`Placement: ${treatmentData.placement}`, margin + 5, 11);
    addText(`Defect: ${treatmentData.defectType}`, margin + 5, 11);
    addText(`Description: ${treatmentData.defectDescription}`, margin + 5, 11);
    yPos += 5;

    // Materials Needed
    addText('MATERIALS NEEDED', margin, 14, true);
    yPos += 3;
    treatmentData.materialsNeeded.forEach((material, index) => {
    ...
    yPos += 3;
    treatmentData.treatmentSteps.forEach((step, index) => {
    ...
    if (treatmentData.maintenanceSchedule && treatmentData.maintenanceSchedule.length > 0) {
      yPos += 5;
      addText('MAINTENANCE SCHEDULE', margin, 14, true);
      yPos += 3;
      treatmentData.maintenanceSchedule.forEach((maintenance, index) => {
    ...
    // Save PDF
    doc.save(`${planTitle.replace(/[^a-z0-9]/gi, '_')}_Treatment_Plan.pdf`);
  };
        ...
        <div className={styles.headerBanner}>
          <div className={styles.headerTop}>
            ...
            <div className={styles.headerActions}>
              <button className={styles.downloadPdfButton} onClick={handleDownloadPDF}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
